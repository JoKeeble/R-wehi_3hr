...
source("setup.R")
cytokines <- read.csv("data/cytokines.csv", header = TRUE)
str(cytokines)
```
...
frame (`cytokines`), and the subsequent arguments are the columns to keep.

```{r, results = 'hide', purl = FALSE}
select(cytokines, patient_id, group, IL17F)
```

To choose rows, use `filter()`:

```{r, purl = FALSE}
filter(cytokines, group == infected)
```
